...
class ImprovementOperatorKnowledgeStackAssetsTests(unittest.TestCase):
    ...
    def test_knowledge_bootstrap_route_workflow_template_present(self) -> None:
        ...
        self.assertIn("jq -r --arg current \"${CURRENT_RUN_ID}\"", content)
        self.assertNotIn("python3 -c 'import json, os, sys", content)
        ...
        self.assertIn("Restore prior evidence lookup runtime history artifact", content)
        self.assertIn("evidence_lookup_runtime_history.jsonl", content)
        self.assertIn("Seed baseline hypothesis templates", content)
        self.assertIn("improvement seed-hypotheses", content)
        self.assertIn("improvement_hypothesis_templates_knowledge_stack.json", content)
        self.assertIn("seed_hypotheses_report.json", content)
        ...
        self.assertIn("EVIDENCE_LOOKUP_READY", content)
        self.assertIn("BATCH_OUTPUT_PATH", content)
        self.assertIn("DEFAULT_REPORT_PATH", content)
        self.assertIn('"${EVIDENCE_LOOKUP_READY:-0}" != "1"', content)
        self.assertIn('"${EVIDENCE_LOOKUP_COMMAND:-none}" == "none"', content)
        self.assertIn('REPORT_PATH="${EVIDENCE_LOOKUP_REPORT_PATH:-}"', content)
        self.assertIn('if [[ -z "${REPORT_PATH}" || "${REPORT_PATH}" == "none" ]]', content)
        self.assertIn('REPORT_PATH="${BATCH_OUTPUT_PATH}"', content)
        self.assertIn('REPORT_PATH="${DEFAULT_REPORT_PATH}"', content)
        self.assertIn("Missing evidence lookup batch output report", content)
        self.assertIn("Missing evidence lookup report", content)
        ...
        self.assertIn("id: guardrail", content)
        ...
        self.assertIn("Guardrail diagnostics", content)
        self.assertIn("steps.guardrail.outcome == 'failure'", content)
        self.assertIn("Top daily pipeline errors (first 5):", content)
    ...
    def test_codeowner_review_gate_audit_script_present(self) -> None:
        ...
        self.assertIn("--event-audit-since", content)
    ...
    def test_reconcile_codeowner_review_gate_drift_check_workflow_template_present(self) -> None:
        ...
        self.assertIn("reconcile-codeowner-review-gate-drift-check-${{ github.ref }}", content)
        ...
        self.assertIn("Fail when reconcile admin token is missing", content)
        self.assertIn("Missing JARVIS admin token", content)
    ...
    def test_reconcile_codeowner_review_gate_audit_workflow_template_present(self) -> None:
        ...
        self.assertIn("schedule:", content)
        self.assertIn("concurrency:", content)
        self.assertIn("reconcile-codeowner-review-gate-audit-${{ github.ref }}", content)
        self.assertIn("cancel-in-progress: true", content)
        self.assertIn("timeout-minutes: 15", content)
        ...
        self.assertIn("--event-audit-since 2026-04-23T00:00:00Z", content)
        self.assertIn("Fail when reconcile admin token is missing", content)
        self.assertIn("Missing JARVIS admin token", content)
    ...
    def test_active_reconcile_codeowner_workflow_present(self) -> None:
        ...
        self.assertIn("concurrency:", content)
        self.assertIn("reconcile-codeowner-review-gate-${{ github.ref }}", content)
        self.assertIn("cancel-in-progress: true", content)
        self.assertIn("timeout-minutes: 15", content)
