...
class ImprovementOperatorKnowledgeStackAssetsTests(unittest.TestCase):
    ...
    def test_reconcile_codeowner_review_gate_drift_check_workflow_template_present(self) -> None:
        ...
        self.assertIn("--required-status-check improvement-gate-status-compact", content)
        self.assertIn("--required-status-check improvement-knowledge-bootstrap-route", content)
        self.assertIn("--required-status-check improvement-domain-smoke-nightly", content)
        self.assertIn("--required-status-check improvement-controlled-matrix-nightly", content)
        self.assertIn("--required-status-check improvement-evidence-lane-smoke", content)
        self.assertIn("--source-workflow-run-id", content)
        self.assertIn("--source-workflow-run-conclusion", content)
        self.assertIn("--source-workflow-name", content)
        self.assertIn("--source-workflow-event", content)
